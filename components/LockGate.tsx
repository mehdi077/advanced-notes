...
export default function LockGate({ children }: { children: React.ReactNode }) {
  ...
  const handleBackspace = useCallback(() => {
    ...
    if (setStep === 1) setPin(p => p.slice(0, -1));
  ...
  const handleDigit = useCallback(
    (d: string) => {
      ...
      if (setStep === 1) setPin(prev => (prev.length >= 6 ? prev : prev + d));
      else setConfirmPin(prev => (prev.length >= 6 ? prev : prev + d));
    },
    [configured, setStep],
  );

  const submit = useCallback(
    async (finalPin: string, finalConfirmPin?: string) => {
  ...
  }, [unlockToken, configured, pin, confirmPin, setStep, isSubmitting, submit]);
  ...
  }, [unlockToken, configured, pin, isSubmitting, handleBackspace, handleClear, handleDigit, submit]);
      ...
      <div className="fixed inset-0 z-[99999] bg-black text-white flex items-center justify-center px-6 py-10">
        <div className="w-full max-w-sm">
          ...
          <div className="mt-6 grid grid-cols-3 gap-4 justify-items-center">
            {keypad.map((k, idx) => {
              if (k.kind === 'empty') return <div key={idx} className="h-16 w-16" />;
